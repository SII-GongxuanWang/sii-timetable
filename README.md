...
默认勾选“记住此设备”，解锁成功后在该浏览器的 IndexedDB 中保存不可导出的 PBKDF2 CryptoKey，有效期 30 天。刷新、关闭后重开均自动解锁；课表更新仍可使用原口令对应的密钥。不会保存口令字符串或明文课表。此设备的浏览器因而具备解密能力，仅在自己的设备上勾选；“锁定”会删除保存的密钥，并通知同浏览器其他打开的课表页面锁定。口令变更、到期或清除网站数据后需重新输入。浏览器无法使用持久存储时退回仅本次查看。
